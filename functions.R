...
dspan <- function(t_start, t_end) {
  # function to generate dbreaks, dlabels, and xlabel for ggplotting time data
  # configure breaks and labels appropriately for span from t_start and t_end
  # looking for approx 8 - 12 breaks across span of x-axis
  # as t_start and t_end posix times
  ...
  # build list containing dbreaks, dlabels, xlabel to return from function    
  span <- list(dbreaks = dbreaks, dlabels = dlabels, xlabel = xlabel)
...
plot_shower <- function (s=study, l=logging, k=KEYCODE, DT=DT_shower_interval4, 
  ...
  if(save.charts) {
    ...
    ggsave(p2,path=wd_charts,file=paste0('shower_',k,'_',pdate,".png"),
           width=10,height=7)
    
    # save to (giant) pdf file
    ggsave(p2,path=wd_charts,file=paste0('shower_',k,'_',pdate,".pdf"),
           width=20,height=14)
...
plot_water <- function (s=study, l=logging, k=KEYCODE, DT=DT_shower_interval4, 
                         t1, t2, save.charts=FALSE) {
  # function to plot total and hot water flow for one slk for a time span
  # modified from plot_shower(), left plot_shower() for prior scripts
  # s = study               - Seattle | EBMUD
  # l = logging             - 1 | 2 | 3 , the phase of the study
  # k = KEYCODE             - 5 digit integer that identifies site
  # DT=DT_shower_interval4  - information about shower interval data
  ...
  # load the hot water Flow data as a data.table
  DT_hw_flows <- get_table(fn_database = hw_file, db_table = 'Flows')
  ...
  # restrict the total water and hot water flows to just the desired times
  DT_tw_flows <- DT_tw_flows[date.time>=t_start & date.time<=t_end, list(Rate,meter),by="date.time"]  
  DT_hw_flows <- DT_hw_flows[date.time>=t_start & date.time<=t_end, list(Rate,meter),by="date.time"]  
  ...
  # if it's only hot water, draw a red rectangle   
  # if it's only total water, draw a blue rectangle
  # if it's both, draw a purple rectangle where they overlap
  # draw a blue rectangle where total water is greater than hot water
  # draw a red rectangle where hot water is greater than total water  
  ...
  p2 <- p2 + xlab(span$xlabel) +
    ylab("total[blue] / hot[pink] (GPM) ") +
    ggtitle(paste0("water flow ",k))
  
  # titles and subtitles
  plot.title = paste0("water flows for house ",k)
  pdate = strftime(t_start, "%a %F")
  plot.subtitle = paste0('date = ', pdate)
  p2 <- p2 + ggtitle(bquote(atop(.(plot.title),scriptstyle(.(plot.subtitle))))) 
  ...
  if(save.charts) {
    # save to (giant) png file
    ggsave(p2,path=wd_charts,file=paste0('water_',k,'_',pdate,".png"),
           width=10,height=7)
    
    # save to (giant) pdf file
    ggsave(p2,path=wd_charts,file=paste0('water_',k,'_',pdate,".pdf"),
           width=20,height=14)
    # the PDF format shows the short interval draws.
  }
